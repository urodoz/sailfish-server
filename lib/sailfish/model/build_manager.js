...
var buildManagerClass = function(models) {
    ...
    var buildModel = models["model.Build"];

    this.getNextSequence = function(projectName, callback) {
        buildModel.find({"project": projectName}).limit(1).sort({datetime:-1}).exec(function(err, builds) {
            var nextSeq = 1;
    ...
    this.getLatestBuild = function(projectName, callback) {
        buildModel.find({"project": projectName, "success": true}).limit(1)
